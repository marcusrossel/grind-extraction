...
namespace Lean.Meta.Grind.Extraction.NaiveAStar
...
abbrev _root_.Lean.Meta.Grind.Extraction.NaiveAStar.TopDownM :=
  StateT State <| ReaderT Context GoalM
...
def visitAppNode (cnode : CostNode) : TopDownM Unit := do
  ...
  cnode.expr.withApp fun fn args => do
    ...
    -- **TODO** What if all children are non-internalized? Then we should again treat `cnode` like
    --          a leaf node.
...
abbrev _root_.Lean.Meta.Grind.Extraction.NaiveAStar.BottomUpM :=
  StateT State <| ReaderT Context GoalM
...
abbrev _root_.Lean.Meta.Grind.Extraction.NaiveAStar.ConstructM :=
  StateT State <| ReaderT Context <| GoalM
